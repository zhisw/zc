...
function layuiXtree(options) {
    ...
    this.dataBind(this._dataJson,this.hadRole);
    this.Rendering();

}

//生产结构
layuiXtree.prototype.dataBind = function (d,hadRole) {
    var _this = this;
    var checkStr='';
    var checkStr2='';
    if (d.length > 0) {
        for (i in d) {
            ...
            if(hadRole.length>0){
            	for(var a=0;a<hadRole.length;a++){
            		 if(hadRole[a]==d[i].id){
            			checkStr= '<input type="checkbox" checked="checked" class="layui-xtree-checkbox" ' + xtree_isend + ' value="' + d[i].id + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck">';
            			break;
            		}else{
            			checkStr= '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + ' value="' + d[i].id + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck">';
            		}
            	}
            }else{
            	checkStr= '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + ' value="' + d[i].id + '" title="' + d[i].name + '" lay-skin="primary" lay-filter="xtreeck">';
            }
            _this._domStr += checkStr;
            _this.dataBind(d[i].children,hadRole);
            _this._domStr += '</div>';
        }
    }
}
