...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		...
		$.get(url,function(data){
			...
			if($(".navBar").html() == ''){
				var _this = this;
				$(".navBar").html(navBar(data)).height($(window).height()-60);
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				title += '<i class="layui-icon layui-unselect layui-tab-refresh" data-id="'+tabIdIndex+'">&#x1002;</i>';
				title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="'+tabIdIndex+'">&#x1006;</i>';
				element.tabAdd(tabFilter, {
			        title : title,
			        content :"<iframe src='"+_this.attr("data-url")+"' data-id='"+tabIdIndex+"' id='"+tabIdIndex+"'></frame>",
			        id : new Date().getTime()
			    })
				//当前窗口内容
				var curmenu = {
		...
		$(window).on("resize",function(){
			...
			if(topTabsWidth > topTabsBoxWidth){
				if(tabLi.position().left > topTabsBoxWidth || tabLi.position().left+topTabsBoxWidth > topTabsWidth){
					...
					window.sessionStorage.setItem("topLeft",topTabsBoxWidth-topTabsWidth);
					$('.rightArrow').width(40+'px')
					$('.closeBox').css({'padding':0})
					$('.rightArrowChilren').show()
				}else{
					topTabs.css("left",-tabLi.position().left);
					window.sessionStorage.setItem("topLeft",-tabLi.position().left);
				}
				//拖动效果
				var flag = false;
				var cur = {
				...
				//隐藏左右箭头
				$('.rightArrow').width(0+'px')
				$('.leftArrow').hide()
				$('.closeBox').css({'paddingLeft':20+'px','paddingRight':20+'px'})
				$('.rightArrowChilren').hide()
				//移除pc端拖动效果
