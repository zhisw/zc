...
*/
	...
	if(window.sessionStorage.getItem("menu") != null){
		...
		for(var i=0;i<menu.length;i++){
			...
			openTitle += '<i class="layui-icon layui-unselect layui-tab-refresh" data-id="'+menu[i].layId+'">&#x1002;</i>';
			openTitle += '<i class="layui-icon layui-unselect layui-tab-close" data-id="'+menu[i].layId+'">&#x1006;</i>';
			element.tabAdd("bodyTab",{
				title : openTitle,
		        content :"<iframe src='"+menu[i].href+"' data-id='"+menu[i].layId+"' id='"+menu[i].layId+"'></frame>",
		        id : menu[i].layId
			})
			//定位到刷新前的窗口
    ...
    $(".layui-tab-refresh").on('click',function(){
    	...
    	document.getElementById(dataid).contentWindow.location.reload(true);
    	//.location.reload();
    })
	//刷新当前
...
//打开新窗口
function addTab(_this,idCheck){
	tab.tabAdd(_this);
	/*zsw---add*/
	if(idCheck!=undefined){
			$('iframe').each(function(){
			if($(this).attr('src')=='page/consumable/ConsumableInventory.html'){
				if($(this)[0].contentWindow.active){
					$(this)[0].contentWindow.active.setTop('detail',idCheck)
				}
			}
			})
	}
